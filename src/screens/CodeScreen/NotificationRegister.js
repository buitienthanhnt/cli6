...
const NotificationRegister = props => {
  ...
  const {mutate: registerNotification, isLoading, isSuccess} = useRegisterFcm();
  ...
  return (
    <View
      style={{flex: 1, padding: 6, backgroundColor: 'rgba(0, 114, 0, 0.5)'}}>
      ...
      <Tooltip
        popover={<Text>coppied to Clipboard: </Text>}
        withOverlay={false}
        skipAndroidStatusBar={true}
        onOpen={() => {
          copyToClipboard(`${fcmtoken}`);
        }}>
        <Text style={{color: '#dd5fc0'}}>
          <Icon name="copy" size={18} color="tomato" /> {fcmtoken}
        </Text>
      </Tooltip>
      <View>
        <Text>
          deviceId: {DeviceInfo.getDeviceId()} {props.g_data.number}
        </Text>
      ...
      <TouchableOpacity
        style={{
          alignItems: 'center',
          backgroundColor: 'rgba(53, 102, 142, 0.4)',
          borderRadius: 6,
          padding: 6,
        }}
        onPress={registerNotification}>
      ...
      <View style={{height: 2, backgroundColor: 'black', marginVertical: 4}} />
  ...
  return (
    <StyledView style={{flex: 1, paddingBottom: 10}}>
      <StyledText
        ...
        List notification: {props.g_data.number}g
      </StyledText>
      <FlatList
        data={data}
        extraData={data}
          ...
          return (
            <View style={{flexDirection: 'row', marginVertical: 4}}>
              ...
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: 'rgba(255, 203, 0, 0.6)',
