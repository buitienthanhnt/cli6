...
const DemoUseCallBack = () => {
    ...
    const callApi = useCallback((pageParam) => {
        ...
        return fetch(`https://reqres.in/api/users?page=${pageParam}`).then((response) => {
    ...
    const onChange = useCallback(debounce((text) => {
        setText(text)
    }, 1000), [text])
        ...
        <View>
            ...
            <View className={'flex-row items-center'}>
                ...
                <TouchableOpacity className={'ml-2'} onPress={() => {
                    setVal('');
                    setText('');
                }}><Text className={'text-lg font-semibold'}>X</Text></TouchableOpacity>
            </View>
            <Text>{text}</Text>
            {
            ...
            <Button title='call api by mutate' onPress={() => {
